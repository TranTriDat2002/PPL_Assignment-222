...
class ASTGeneration(MT22Visitor):
    ...
    def visitIntlitlist(self, ctx:MT22Parser.IntlitlistContext):
        ...
        return [IntegerLit(int(ctx.INTLIT()))] + self.visit(ctx.intlitlist())
    ...
    def visitStringexpr(self, ctx:MT22Parser.StringexprContext):
        if ctx.getChildCount() == 1:
            return self.visit(ctx.getChild(0))
        return BinExpr(ctx.STRINGCONCAT().getText(),self.visit(ctx.stringexpr(0)),self.visit(ctx.stringexpr(1)))
